...
fn answer(input: Vec<String>) -> usize {
    ...
    // find the guard/minute pair that is most often in a sleep state
    let (guard, minute) = regularest_guard_minute(&asleep);

    // multiply the two together and return the result
    guard * minute
}

fn regularest_guard_minute(asleep: &HashMap<usize, HashMap<usize, usize>>) -> (usize, usize) {
    let (guard, (minute, _)) = asleep
        // make iterator of (guard, minute count map) tuples
        .iter()
        // convert to iterator over (guard, (minute, max count)) tuples
        .map(|(guard, minute_counts)| {
            (
                guard,
                minute_counts
                    // make iterator of (minute, count) tuples
                    .iter()
                    // find minute with highest sleep count
                    .max_by_key(|x| x.1)
                    .unwrap(),
            )
        })
        // find guard with minute with highest sleep count
        .max_by_key(|x| (x.1).1)
        .unwrap();
